...
function MakeClaimModal({ onClose }) {
  ...
  try {
    const { data: users } = await axios.get('http://localhost:4000/admin/listusers');
    ...
    if (user && user.selectedPolicies) {
      const policyDetailsRequests = user.selectedPolicies.map(policyId =>
        axios.get(`http://localhost:4000/user/policies/${policyId}`)
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:4000/user/claims/create', {
        ...formData,
